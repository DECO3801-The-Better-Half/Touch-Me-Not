...
from constants import DEFAULT_PORT_ONE, DEFAULT_PORT_TWO, USERS_FILE, USAGE, \
    NUM_AUDIO_CHANNELS, BASE_THRESHOLD, CAPACITANCE_OVERFLOW, TICKS_PER_SECOND
        ...
        for i, arg in enumerate(sys.argv[1:]):
            ...
            if arg.startswith("-"):
                ...
                if "k" in arg:
                    ...
                    use_keyboard = True
                    print(
                        "Using keyboard: numbers 0-9 represent object touches")
                if "m" in arg:
                    ...
                    try:
                        ...
                        exit(1)
                    print(
                        f"Using modulation: {modulation_cooldown_period}s cooldown period")
            else:
                # Check for username
                ...
                if arg.lower() in users:
                    ...
                    create_new_user = input(
                        f"User {arg} not found. Create new user? (y/n): ").lower() == "y"
                    if create_new_user:
                        # Create new user
                        port_one = input(
                            "Enter port for first arduino: e.g. \"/dev/cu.usbserial-10\": ")
                        port_two = input(
                            "Enter port for second arduino: e.g. \"/dev/cu.usbmodem1101\": ")
                        users[arg.lower()] = {"port_one": port_one,
                                              "port_two": port_two}
                        # Update current port settings
                        port_one = users[arg]["port_one"]
                        port_two = users[arg]["port_two"]
                    ...
                    for cur_instrument, value in data.items():
                        if value >= cur_instrument.threshold or value == CAPACITANCE_OVERFLOW:
                            ...
                            if use_modulation and time.time() - last_modulation_time > modulation_cooldown_period:
                                old_key = current_key
                                current_key = instructions.get_music_key(
                                    cur_instrument)
                                if old_key != current_key:
                                    # Modulate
                                    Modulator.modulate(old_key, current_key,
                                                       all_instruments,
                                                       cur_instrument)
                                    last_modulation_time = time.time()
                ...
                if event.type == pygame.QUIT:
                    ...
                    if number_pressed in range(0, 10):
                        ...
                        if use_modulation and time.time() - last_modulation_time > modulation_cooldown_period:
                            old_key = current_key
                            current_key = instructions.get_music_key(
                                all_instruments[number_pressed])
                            if old_key != current_key:
                                Modulator.modulate(old_key, current_key,
                                                   all_instruments,
                                                   all_instruments[
