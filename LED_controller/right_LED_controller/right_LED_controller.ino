...
int water [] = {12,13,14,15,28,29,30,31,44,45,46,47,60,61,62,63,76,77,78,79,92,93,94,95};

void setup() {
Serial.begin(9600);
strip.begin(); //always needed
strip.show(); // 0 data => off.
strip.setBrightness(125); // ~50%
...
//read all sensor values
 for (int i = 0; i < NUMBER_OF_SENSORS; i = i + 1) {
      ...
      sensors[i].set_CS_Timeout_Millis(100);  // Lowered from 500
      sensors[i].set_CS_AutocaL_Millis(10000);
    }
//plant 1 LED response
led_response(values[0], thresholds[0], water, blue);

//plant 2 LED response
led_response(values[1], thresholds[1], plant2, light_green);

//flower LED response
led_response(values[2], thresholds[2], plant1, green);

//water LED response
led_response(values[3], thresholds[3], flower, purple);
